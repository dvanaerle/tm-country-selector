  ...
  "relative w-full rounded border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        ...
        success:
          "bg-green-50 text-green-600 [&>svg]:text-current *:data-[slot=alert-description]:text-green-600/90",
        error:
          "bg-red-50 text-red-600 [&>svg]:text-current *:data-[slot=alert-description]:text-red-600/90",
      },
    },
    defaultVariants: {
      variant: "default",
...
function AlertDescription({
  ...
  return (
    <div
      ...
      className={cn(
        "text-green col-start-2 grid justify-items-start gap-1 text-sm [&_p]:leading-relaxed",
        className,
      )}
      {...props}
    />
